...
import numpy as np
import threading
from onix.headers.quarto_frequency_lock_two_loop import Quarto
...
with device_lock:
    initial_p_gain = q.get_p_gain()

p_gain.setValue(initial_p_gain)
p_gain.setDecimals(3)
p_gain.setSingleStep(0.001)
p_gain.editingFinished.connect(_p)
p_gain.setMinimum(-10)
p_gain.setMaximum(10)
p_gain_proxy = QtWidgets.QGraphicsProxyWidget()
p_gain_proxy.setWidget(p_gain)
win.addItem(p_gain_proxy, row = 7, col = 0)
...
with device_lock:
    initial_i_time = q.get_i_time()

i_time.setValue(initial_i_time)
i_time.setDecimals(0)
i_time.setSingleStep(1)
i_time.editingFinished.connect(_i)
i_time.setMinimum(0)
i_time.setMaximum(1e8)
i_time_proxy = QtWidgets.QGraphicsProxyWidget()
i_time_proxy.setWidget(i_time)
win.addItem(i_time_proxy, row = 7, col = 1)
...
with device_lock:
    initial_d_time = q.get_d_time()

d_time.setValue(initial_d_time)
d_time.setDecimals(1)
d_time.setSingleStep(0.1)
d_time.editingFinished.connect(_d)
d_time.setMinimum(0)
d_time.setMaximum(1e8)
d_time_proxy = QtWidgets.QGraphicsProxyWidget()
d_time_proxy.setWidget(d_time)
win.addItem(d_time_proxy, row = 7, col = 2)
win.nextRow()

## Cavity P
def _p_cavity():
    with device_lock:
        q.set_p_gain_cavity(p_gain_cavity.value())

p_gain_cavity = QtWidgets.QDoubleSpinBox(prefix = "P_Cavity: ")
with device_lock:
    initial_p_gain_cavity = q.get_p_gain_cavity()

p_gain_cavity.setValue(initial_p_gain_cavity)
p_gain_cavity.setDecimals(3)
p_gain_cavity.setSingleStep(0.001)
p_gain_cavity.editingFinished.connect(_p_cavity)
p_gain_cavity.setMinimum(-10)
p_gain_cavity.setMaximum(10)
p_gain_cavity_proxy = QtWidgets.QGraphicsProxyWidget()
p_gain_cavity_proxy.setWidget(p_gain_cavity)
win.addItem(p_gain_cavity_proxy, row = 8, col = 0)

## Cavity I
def _i_cavity():
    with device_lock:
        q.set_i_time_cavity(i_time_cavity.value())

i_time_cavity = QtWidgets.QDoubleSpinBox(prefix = "I_Cavity: ")
with device_lock:
    initial_i_time_cavity = q.get_i_time_cavity()

i_time_cavity.setValue(initial_i_time_cavity)
i_time_cavity.setDecimals(0)
i_time_cavity.setSingleStep(1)
i_time_cavity.editingFinished.connect(_i_cavity)
i_time_cavity.setMinimum(0)
i_time_cavity.setMaximum(1e8)
i_time_cavity_proxy = QtWidgets.QGraphicsProxyWidget()
i_time_cavity_proxy.setWidget(i_time_cavity)
win.addItem(i_time_cavity_proxy, row = 8, col = 1)

## Cavity D
def _d_cavity():
    with device_lock:
        q.set_d_time_cavity(d_time_cavity.value())

d_time_cavity = QtWidgets.QDoubleSpinBox(prefix = "D_Cavity: ")
with device_lock:
    initial_d_time_cavity = q.get_d_time_cavity()

d_time_cavity.setValue(initial_d_time_cavity)
d_time_cavity.setDecimals(1)
d_time_cavity.setSingleStep(0.1)
d_time_cavity.editingFinished.connect(_d_cavity)
d_time_cavity.setMinimum(0)
d_time_cavity.setMaximum(1e8)
d_time_cavity_proxy = QtWidgets.QGraphicsProxyWidget()
d_time_cavity_proxy.setWidget(d_time_cavity)
win.addItem(d_time_cavity_proxy, row = 8, col = 2)
