...
class M4i6622:
    def __init__(self, address=b'/dev/spcm0',channelNum = 4, referenceClock=False, referenceClockFrequency = 10000000, clockOut = False, pulseLength = 2e-3, sampleRate = 625, sequenceReplay = False, singleTrigger = False, multiTrigger = False, extTrigger = False, singleRestart = False, loop = 1, numSegments = 1, verbose = False, trigger = False,  loop_list = [], next = [], conditions = [], initialStep = 0, segTimes = [], triggerLevel = 2000, filters = [0,0,0,0], amplitudes = [2500,2500,2500,2500], ttlOutOn=False, ttlOutChannel=[0], ttlSourceChannel=0):
        """
        ...
        if self.sequenceReplay: #for sequenceReplay, pulseLength is not a parameter, so we add the lengths of each segment instead.
        ...
        if self.ttlOutOn:
            ...
            for i in range(3):
                val = spcm_dwSetParam_i32 (self.hCard, SPCM_X0_MODE + i, SPCM_XMODE_DISABLE)
                if verbose: print(f'TTL channel {i} disabled with error code {val}.')
        ...
        if self.referenceClock == True:
            val = spcm_dwSetParam_i32 (self.hCard, SPC_CLOCKMODE, SPC_CM_EXTREFCLOCK); # Set to reference clock mode
            if val != 0: print(f"Error when setting reference clock, error code: {val}\n")
            val = spcm_dwSetParam_i32 (self.hCard, SPC_REFERENCECLOCK, referenceClockFrequency); # Reference clock that is fed in at the Clock Frequency
            if val != 0: print(f"Error when setting reference clock frequency, error code: {val}\n")
        else:
            if self.verbose: print("Using internal clock\n")
        ...
        for i in range(0, self.channelNum, 1):
            ...
            spcm_dwSetParam_i32 (self.hCard, amplitudeList[i], int32(amplitudes[i])) # Setting the max amplitude
            spcm_dwSetParam_i32 (self.hCard, filterList[i], int32(filters[i])) #Setting the channel's filter
        ...
        if val!=0 or val1 != 0: print(f"Errors {val}, {val1} when setting trigger")

        #Setup the trigger
        if trigger:
            val = spcm_dwSetParam_i32 (self.hCard, SPC_TRIG_ORMASK, SPC_TMASK_EXT0)
            if val != 0: print(f"Error {val} when setting trigger")
            ...
            if val != 0 or val1 != 0 or val2 !=0 or val3 !=0 : print(f"Errors {val}, {val1}, {val2}, {val3} when setting trigger")

            # set mode and level of trigger
            val = spcm_dwSetParam_i32 (self.hCard, SPC_TRIG_EXT0_MODE, SPC_TM_POS)
            val1 = spcm_dwSetParam_i32 (self.hCard, SPC_TRIG_EXT0_LEVEL0, triggerLevel) # in mV
            val2 = spcm_dwSetParam_i32 (self.hCard, SPC_TRIG_TERM, 1) # 50 Ohm termination
            val3 = spcm_dwSetParam_i64 (self.hCard, SPC_TRIG_DELAY, 0)
            if val != 0 or val1 != 0 or val2 !=0 or val3 !=0 : print(f"Errors {val}, {val1}, {val2}, {val3} when setting trigger")
            if self.verbose: print("Trigger mode set to external")

        else:
            val = spcm_dwSetParam_i32 (self.hCard, SPC_TRIG_ORMASK, SPC_TMASK_SOFTWARE)
            if val != 0: print(f"Error {val} when setting trigger")
            if self.verbose: print("Trigger mode set to software")
        ...
        """
        self.llNumSamplesPerChannel = int(np.floor(self.numSamples / self.channelNum)) #number of samples needed to represent the wave on one channel
        self.llNumSamplesPerChannel = int(self.llNumSamplesPerChannel + (32 - self.llNumSamplesPerChannel % 32)) #round to 32 bit integer
        self.llLoops = int(loop)
        self.segSize = int(np.floor(self.numSamples / (self.numSegments * self.channelNum)))
        self.segmentSize = int(self.segSize + (16 - self.segSize % 16)) #get the size of each segment (when applicable) and round to 16 bit integer


        #set the mode
        if sequenceReplay:
            val = spcm_dwSetParam_i32 (self.hCard, SPC_CARDMODE,    SPC_REP_STD_SEQUENCE)
            if self.verbose: print("Sequence Replay mode loaded w/ code : " + str(val))
            logsegments = int(np.ceil(math.log2(len(self.segTimes)))) #we must divide memory into a power of 2 number of segments
            val1 = spcm_dwSetParam_i32 (self.hCard, SPC_SEQMODE_MAXSEGMENTS, int32 (2**logsegments))
            val2 = spcm_dwSetParam_i32 (self.hCard, SPC_SEQMODE_STARTSTEP, self.initialStep) #specified step is the first step after you start the card
            if val1 !=0 or val2 != 0: print(f"Initial sequenceReplay settings done with errors: {val1}, {val2}")

        elif singleTrigger:
            val = spcm_dwSetParam_i32 (self.hCard, SPC_CARDMODE, SPC_REP_STD_SINGLE)
            val1 = spcm_dwSetParam_i64 (self.hCard, SPC_MEMSIZE, self.llNumSamplesPerChannel)
            val2 = spcm_dwSetParam_i64 (self.hCard, SPC_LOOPS, self.llLoops)
            if val != 0 or val1 != 0 or val2 !=0 : print(f"Errors {val}, {val1}, {val2} when setting mode")
            if self.verbose: print("Mode set to singleTrigger")

        elif singleRestart:
            val = spcm_dwSetParam_i32 (self.hCard, SPC_CARDMODE, SPC_REP_STD_SINGLERESTART)
            val1 = spcm_dwSetParam_i64 (self.hCard, SPC_MEMSIZE, self.llNumSamplesPerChannel)
            val2 = spcm_dwSetParam_i64 (self.hCard, SPC_LOOPS, self.llLoops)
            if val != 0 or val1 != 0 or val2 !=0 : print(f"Errors {val}, {val1}, {val2} when setting mode")
            if self.verbose: print("Mode set to singleRestart")
            ...
            if val != 0 or val1 != 0 or val2 !=0 or val3 !=0 : print(f"Errors {val}, {val1}, {val2}, {val3} when setting mode")
            if self.verbose: print("Mode set to multiTrigger")
            ...
            val2 = spcm_dwSetParam_i64 (self.hCard, SPC_LOOPS, self.llLoops)
            if val != 0 or val1 != 0 or val2 !=0 : print(f"Errors {val}, {val1}, {val2} when setting mode")
            if self.verbose: print("Mode set to extTrigger")
    ...
    def setSoftwareBuffer(self):
        ...
        self.qwBufferSize = uint64 (self.numSamples * self.lBytesPerSample.value) #total size of the buffer
                ...
                if i > functionNum - 1:
                    ...
                    if self.ttlOutOn and i == self.ttlSourceChannel:
                        if len(self.ttlOutChannel) == 1:
                            Y_vect = Y_vect + [self.mix_ttl_and_analogue15(X, functionList[i], ttlFunctionList[i])]
                        elif len(self.ttlOutChannel) == 2:
                            Y_vect = Y_vect + [self.mix_ttl_and_analogue14(X, functionList[i], ttlFunctionList[i])]
    ...
    def programTTLOut(self):
        ...
        if len(self.ttlOutChannel) == 2:
            dwXMode = (SPCM_XMODE_DIGOUT | spcm_ttl_source_code | SPCM_XMODE_DIGOUTSRC_BIT15)
            val = spcm_dwSetParam_i32 (self.hCard, SPCM_X0_MODE + self.ttlOutChannel[0], uint32(dwXMode))
            #if self.verbose:
            print(f"TTL out enabled w/ code : {str(val)}. Bit 15 of channel {self.ttlOutChannel} is now used for TTL out.")

            dwXMode = (SPCM_XMODE_DIGOUT | spcm_ttl_source_code | SPCM_XMODE_DIGOUTSRC_BIT14)
            val = spcm_dwSetParam_i32 (self.hCard, SPCM_X0_MODE + self.ttlOutChannel[1], uint32(dwXMode))
            print(f"TTL out enabled w/ code : {str(val)}. Bit 14 of channel {self.ttlOutChannel} is now used for TTL out.")
    ...
    def transferData(self):
        try:
            ...
            if self.verbose: print("Starting the DMA transfer and waiting until data is in board memory\n")
            val = spcm_dwDefTransfer_i64 (self.hCard, SPCM_BUF_DATA, SPCM_DIR_PCTOCARD, uint32(self.qwBufferSize.value), self.trueBuffer, uint64 (0), self.qwBufferSize)
            if val != 0: print(f"Error in data transfer, error code {val}\n")
            if self.verbose: print("Finished initial transfer\n")
            val = spcm_dwSetParam_i32 (self.hCard, SPC_DATA_AVAIL_CARD_LEN, self.qwBufferSize)
            if val != 0: print(f"Error in data transfer, error code {val}\n")
            ...
            if self.verbose: print("Got full data size\n")
            val = spcm_dwSetParam_i32 (self.hCard, SPC_M2CMD, M2CMD_DATA_STARTDMA | M2CMD_DATA_WAITDMA) #transfer the data and wait for it to be done before returning
            if val != 0: print(f"Error in data transfer, error code {val}\n")
            if self.verbose: print("... data has been transferred to board memory\n")
    ...
    def configSequenceReplay(self, segFunctions = [], steps=[], ttlFunctionList=[]):
        ...
        for i in range(len(segFunctions)): #iterate through the segments
            size = self.segTimes[i] * self.sampleRate #gets the number of data points we need to represent this segment
            size = int(size + (32 - size % 32)) #round into allowable value
            #print(f"Size for segment {i} is {size}")
            Y_vect = []
            X = np.arange(0,(int)(size),1)

            for j in range(self.channelNum): #iterate through the channels for each segment
                if j > len(self.segFunctions[i]) - 1: #if we set up more channels than we're using, fill them with 0s
                    Y_vect = Y_vect + [np.zeros(len(X)).astype(int)]
                else:
                    ...
                    if self.ttlOutOn and j == self.ttlSourceChannel:
                        if len(self.ttlOutChannel) == 1:
                            Y_vect = Y_vect + [self.mix_ttl_and_analogue15(X, self.segFunctions[i][j], ttlFunctionList[i])]
                        elif len(self.ttlOutChannel) == 2:
                            Y_vect = Y_vect + [self.mix_ttl_and_analogue14(X, self.segFunctions[i][j], ttlFunctionList[i])]
                    else:
                        Y_vect = Y_vect + [np.array(self.segFunctions[i][j](X)).astype(int)] #evaluate the function on our domain, producing a list of y-values representing the wave
            ...
            val = spcm_dwSetParam_i32 (self.hCard, SPC_SEQMODE_WRITESEGMENT, i) #specifies that we are writing the ith segment
            if val != 0 : print(f"Error in SPC_SEQMODE_WRITESEGMENT, code {val} for segment {i}")

            val = spcm_dwSetParam_i32 (self.hCard, SPC_SEQMODE_SEGMENTSIZE, size) #writes the size of the segment
            if val != 0 : print(f"Error in SPC_SEQMODE_SEGMENTSIZE, code {val} for segment {i}")

            #transfer the data for this segment
            val = spcm_dwDefTransfer_i64 (self.hCard, SPCM_BUF_DATA, SPCM_DIR_PCTOCARD, 0, self.trueBuffer, 0, (int) (size * self.lBytesPerSample.value * self.channelNum))
            if val != 0 : print(f"Error in spcm_dwDefTransfer_i64, code {val} for segment {i}")
            val = spcm_dwSetParam_i32 (self.hCard, SPC_M2CMD, M2CMD_DATA_STARTDMA | M2CMD_DATA_WAITDMA)
            if val != 0 : print(f"Error in spcm_dwSetParam_i32, code {val} for segment {i}")
        ...
        for i in range(len(steps)):
            ...
            llCondition = self.conditions[i]  #condition telling us when to go to this next segment

            llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment) #puts all of these parameters into one int64 bit value
            ...
            val = spcm_dwSetParam_i64 (self.hCard, SPC_SEQMODE_STEPMEM0 + i, int64 (llValue)) #writes the parameters for the ith step in our sequence
            if val != 0: print(f"Error in setting the sequence with code {val} for iteration {i}")
    ...
    def startCard(self, checkClock=True):
        try:
            ...
            if dwError == ERR_CLOCKNOTLOCKED:
                ...
                if dwError != 0: print(f"Error when starting card with code {dwError}")
    ...
    def stop(self):
        ...
        try:
            ...
            if dwError != 0: print(f"Error when stopping card with code {dwError}")
    ...
    def stopWithoutClosing(self): #This worked on one simple test. Need to make sure it reliably works as it is known to be faulty.
        """
        Command to stop the Card. To use card again, need to reinitialize
        """
        #send the stop command
