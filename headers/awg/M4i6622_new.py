...
class M4i6622:
    ...
    def __init__(
        ...
        """
        Testing clock sharing by setting an external clock frequency
        for the second AWG and running the internal clock on the first
        one. Assuming that spcm0 corresponds to channels 0-3 and
        spcm1 to 4-7.
        """
        ...
        else:   # old code contained in this else statement
        ...
        for hcard in self._hcards:
            ...
            for ttl_channel in [0, 1, 2]:
                ...
                mode = mode | getattr(
                    pyspcm, f"SPCM_XMODE_DIGOUTSRC_CH{
                        self._ttl_awg_map[ttl_channel]}"
                )
        ...
        for channel in self._awg_channels:
            self._awg_parameters[channel] = {
                "frequency": 0 * ureg.Hz, "amplitude": 0}
    ...
    def _select_channels(self, hcard, channels: list[CHANNEL_TYPE]):
        if len(channels) == 3:
            raise ValueError(
                "Cannot enable 3 channels. Enable 4 channels instead.")
        value = 0
    ...
    def _set_channel_filter(self, hcard, channel: CHANNEL_TYPE, filter: int = 0):
        ret = pyspcm.spcm_dwSetParam_i32(
            hcard, getattr(pyspcm, f"SPC_FILTER{
                           channel}"), pyspcm.int32(filter)
        )
        if ret != pyspcm.ERR_OK:
    ...
    def _set_mode(
        ...
        if mode == "single":
            ...
            raise ValueError(f"The replay mode {
                             mode} is invalid or not implemented.")
    ...
    def _define_transfer_buffer(
        ...
        self._aligned_buffer[index] = pvAllocMemPageAligned(
            len(data) * self._bytes_per_sample)
        # this variable must maintain a reference after exit.
        data = data.astype(np.int16)
        pyspcm.memmove(
            self._aligned_buffer[index], data.ctypes.data, 2 * len(data))
    ...
    def _get_data_ready_to_transfer(self, hcard) -> int:
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Get data ready to transfer failed with code {ret}.")
        return value.value
    ...
    def _set_data_ready_to_transfer(self, hcard, data_bytes: int):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set data ready to transfer failed with code {ret}.")
    ...
    def _set_external_clock_frequency(self, hcard, clock_frequency: int):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set external clock frequency failed with code {ret}.")
    ...
    def _set_ext0_trigger_rising_edge(self, hcard):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set trigger edge to rising failed with code {ret}.")
    ...
    def _set_ext0_trigger_falling_edge(self, hcard):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set trigger edge to falling failed with code {ret}.")
    ...
    def _set_ext0_trigger_level_high(self, hcard):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set trigger level to high failed with code {ret}.")
    ...
    def _set_ext0_trigger_level_low(self, hcard):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set trigger level to low failed with code {ret}.")
    ...
    def _set_multi_purpose_io_mode(self, hcard, line_number: Literal[0, 1, 2], mode: int):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set multiple purpose io mode failed with code {ret}.")
    ...
    def _get_multi_purpose_io_output(self, hcard) -> tuple[bool, bool, bool]:
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Get multi purpose io output failed with code {ret}.")
    ...
    def _set_multi_purpose_io_output(
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set multi purpose io output failed with code {ret}.")
    ...
    def _set_sequence_max_segments(self, hcard, segments: int):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set sequence max segments failed with code {ret}.")

    def _set_sequence_write_segment(self, hcard, segment_number: int):
        ret = pyspcm.spcm_dwSetParam_i32(
            hcard, pyspcm.SPC_SEQMODE_WRITESEGMENT, pyspcm.int32(
                segment_number)
        )
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set sequence write segment failed with code {ret}.")
    ...
    def _set_sequence_write_segment_size(self, hcard, segment_size: int):
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set sequence write segment size failed with code {ret}.")
    ...
    def _set_sequence_step_memory(
        ...
        next_step: int,
        loops: int,
        end: Literal["end_loop", "end_loop_on_trig",
                     "end_sequence"] = "end_loop",
    ):
        ...
        value = (end << 32) | (loops << 32) | (
            next_step << 16) | segment_number
        ret = pyspcm.spcm_dwSetParam_i64(hcard, register, pyspcm.int64(value))
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Set sequence step memory failed with code {ret}.")
    ...
    def _set_sequence_start_step(self, hcard, start_step_number: int):
        ret = pyspcm.spcm_dwSetParam_i32(
            hcard, pyspcm.SPC_SEQMODE_STARTSTEP, pyspcm.int32(
                start_step_number)
        )
        if ret != pyspcm.ERR_OK:
    ...
    def _get_sequence_current_step(self, hcard) -> int:
        ...
        if ret != pyspcm.ERR_OK:
            raise Exception(
                f"Get sequence current step failed with code {ret}.")
        return value.value
    ...
    def _get_error_information(self, hcard) -> tuple[int, int, str]:
        ...
        ret = pyspcm.spcm_dwGetErrorInfo_i32(
            hcard, pyspcm.byref(error_register), pyspcm.byref(
                error_code), pyspcm.byref(text)
        )
    ...
    def _set_sine_segment(self):
        ...
        for awg_channel in self._awg_channels:
            ...
            segment.add_awg_function(
                awg_channel, AWGSinePulse(frequency, amplitude))
    ...
    def _write_segment(self, segment_number: int, segment: Segment):
        ...
        # the sample size must be a multiple of 32.
        size = (size + 31) // 32 * 32
        if size < 96:
    ...
    def start_sequence(self):
        ...
        # start the first card the last to trigger other cards
        for hcard in reversed(self._hcards):
    ...
    def start_sine_outputs(self):
        ...
        for hcard in self._hcards:
            self._set_sequence_start_step(
                hcard, self._sine_segment_steps[self._next_sine_segment])
