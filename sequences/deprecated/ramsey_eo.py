...
class RamseyEO(Sequence):
    ...
    def add_flop(self, flop_time: Q_, flop_amplitude: int, aom_flop_amplitude: int, wait_time: Q_, phase: float):
        F_state = self._probe_transition[0]
        D_state = self._probe_transition[1]
        frequency = energies["5D0"][D_state] - energies["7F0"][F_state] + self._eo_offset_frequency
        segment = Segment("flop", flop_time)
        aom_pulse = AWGSineTrain(
            flop_time, wait_time, self._switch_aom_frequency, aom_flop_amplitude, [0, 0]
        )
        eom_pulse = AWGSineTrain(
            flop_time, wait_time, frequency, flop_amplitude, [0, phase]
        )
        segment.add_awg_function(self._switch_aom_channel, aom_pulse)
        segment.add_awg_function(self._eo_channel, eom_pulse)
        self.add_segment(segment)
