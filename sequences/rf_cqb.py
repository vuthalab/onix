...
class AWGCQB(AWGFunction):
    ...
    def output(self, times):
        ...
        condlist = [np.logical_or(times < pulse_time, times > pulse_time + delay_time)]
        funclist = [sine_sum, zero]
        return np.piecewise(times, condlist, funclist)

    @property
    def min_duration(self) -> Q_:
        return self._pulse_time * 2 + self._delay_time
...
class RFCQB(Sequence):
    ...
    def _add_rf_segment(self):
        ...
        segment = Segment("rf", pulse_time * 2 + delay_time)
        rf_pulse = AWGCQB(
            pulse_time,
            delay_time,
            frequency_1,
