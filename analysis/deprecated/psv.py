...
def plot(raw_data:int, scale:str ='int', detailed = True):
       """
       ...
       for evnt in seq._segment_steps:
              ...
              if not real_event and detailed:
                     tags.append(f'[{1}v:{running_total}][{1}:{running_total + evtime}] {clean_txt(evnt[0])}')
                     tags.append(f'[{1}:{running_total}]+[{1}:{running_total + evtime}] {round(evnt[1]*cycle_len, 2):#~}')
       ...
       head = ['', {'name': ' ', 'wave': "x6x", 'async': [0, running_total/9,running_total*8/9, running_total], 'data': [f'Data Set: {raw_data}']}]
       if detailed:
              head.append({'name': ' ', 'wave': "0", 'async': [0, running_total]})

       with schemdraw.Drawing():
              logic.TimingDiagram(
                     {'signal': [ head,
                                   [['AWG']+[{'name': f'{awg_channels[i]['name']}', 'wave': tdig[i][0], 'async': tdig[i][1]}
                                   for i in tdig]] + 
                                   [['TTL'] + [{'name': f'{ttl_channels[i]['name']}', 'wave': trig_dig[i][0], 'async': trig_dig[i][1],}
                                   for i in trig_dig]]], 
                     'edge': tags,
