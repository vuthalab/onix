 ...
 * 
 * New Changes: this code runs two PID loops at once
 * The first loop controls the vescent output, which is the same as this code always did
 * The second loop controls the cavity error signal
 * We sum the control signals of these two loops and output to the laser piezo
 * Any variable, x, that is specific to the vescent control loop has an analog, x_cavity, on the cavity error PID loop
 * For example, p_gain applies to the vescent control loop, while p_gain_cavity applies to the cavity control loop
*/
...
// PID parameters for cavity error signal control
float p_gain_cavity = 0.005;
float i_time_cavity = 1000.0; // us
float d_time_cavity = 0.0; // us

// error signal offset
float laser_jump_offset = -6.0;
// error signal offset
float error_offset = 5.468;
float error_offset_cavity = 0.0;
// keeps track of the integral term
float integral = 0.0;
float integral_cavity = 0.0;
...
float previous_error_cavity = -100.0;
...
float integral_lower_warning_cavity = -0.9 * integral_limit_cavity;
float integral_upper_warning_cavity = 0.9 * integral_limit_cavity;
  ...
  if (state > 0) {
    if (wait_lock_index < 0) {
      if (transmission > transmission_unlock_voltage) {
        ...
        last_good_integral_cavity = integral_cavity;
...
void update_pid(bool local_pause_data) {
  ...
  if (auto_relock_on) {
    if (confirm_unlock_index == 0) {
      ...
      output = output_offset + last_good_integral + last_good_integral_cavity;
      confirm_unlock_index = -1;
  ...
  if (feedback_on) {
    ...
    float error_cavity = cavity_error_data[data_index] - error_offset_cavity;
    float proportional_cavity = p_gain_cavity * error_cavity;
    integral_cavity = new_integral(integral_cavity, p_gain_cavity * ADC_INTERVAL / i_time_cavity * error_cavity);
    float differential_cavity = 0.0;
    if (previous_error_cavity > -99.0) {  // not the first data point.
      differential_cavity = p_gain_cavity * d_time_cavity / ADC_INTERVAL * (current_error_cavity - previous_error_cavity);
    }
    previous_error_cavity = error_cavity + error_offset_cavity;

    output += proportional + integral + differential + proportional_cavity + integral_cavity + differential_cavity;
...
void cmd_p_gain_cavity(qCommand& qC, Stream& S){
  if ( qC.next() != NULL) {
    p_gain_cavity = atof(qC.current());
  }
  S.printf("p gain cavity is %f\n", p_gain_cavity);
}

void cmd_i_time_cavity(qCommand& qC, Stream& S){
  if ( qC.next() != NULL) {
    i_time_cavity = atof(qC.current());
  }
  S.printf("i time cavity is %f\n", i_time_cavity);
}

void cmd_d_time_cavity(qCommand& qC, Stream& S){
  if ( qC.next() != NULL) {
    d_time_cavity = atof(qC.current());
  }
  S.printf("d time cavity is %f\n", d_time_cavity);
}

void cmd_integral_limit_cavity(qCommand& qC, Stream& S){
  if ( qC.next() != NULL) {
    integral_limit_cavity = atof(qC.current());
  }
  S.printf("integral limit cavity is %f\n", integral_limit_cavity);
  integral_lower_warning_cavity = -0.9 * integral_limit_cavity;
  integral_upper_warning_cavity = 0.9 * integral_limit_cavity;
}

void cmd_error_offset_cavity(qCommand& qC, Stream& S){
  if ( qC.next() != NULL) {
    error_offset_cavity = atof(qC.current());
  }
  S.printf("error offset cavity is %f\n", error_offset_cavity);
}
...
void setup(void) {
  ...
  qC.addCommand("p_gain_cavity", cmd_p_gain_cavity);
  qC.addCommand("i_time_cavity", cmd_i_time_cavity);
  qC.addCommand("d_time_cavity", cmd_d_time_cavity);
  qC.addCommand("integral_limit_cavity", cmd_integral_limit_cavity);
  qC.addCommand("error_offset_cavity", cmd_error_offset_cavity);
