...
Currently you can setup up segment number and segmnet length
Quarto will record this amount of data on input 1 and input 2
When you call data, quarto will print length of data collected from channel 1, then will print ch1 data, then ch2 data
*/
#include "qCommand.h"
#include <math.h>
qCommand qC;
...
const int DATA_LENGTH = 50000; //32767; //100000; // each channel can take a maximum of 32767 points
float data[DATA_LENGTH];
float data2[DATA_LENGTH];
int data_index = 0;
...
void cmd_data(qCommand& qC, Stream& S) {
  ...
  if (running) {
    ...
    S.println(data_index); // TODO: to improve speed, consider sending bits over, and converting to ASCII on comptuer
